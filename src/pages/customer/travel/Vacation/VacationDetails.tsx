...
const DOC_CHECKS = ["Passport"];
const DOC_ALIAS: Record<string,string[]> = { "Passport":["passport"] };
const CATEGORY_LABELS: Record<string,string> = {
  personal:"Personal & Passport", emergency:"Emergency Contact",
};
const ACCOMMODATION_OPTIONS = ["Hotel","Hostel","Airbnb","Resort","Villa","Guesthouse","Other"];
  ...
  <Box>
    <Box sx={{display:"flex",alignItems:"center",gap:1.5,mb:2.5}}>
      ...
      <Box>
        ...
        <Typography sx={{fontSize:12,color:C.g400}}>You've submitted an application for this vacation.</Typography>
      </Box>
    </Box>
...
const SmartApplyPanel: React.FC<PanelProps> = ({offer,profile,clientDocs,offerId,onSuccess}) => {
  ...
  const [form,setForm] = useState({
    destination:"", travel_date:"", number_of_people:"1", accommodation_type:"",
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append("destination",                form.destination||offer?.destination||offer?.location||"");
      fd.append("travel_date",                form.travel_date);
      fd.append("number_of_people",           form.number_of_people||"1");
      ...
      await api.post("/app/vacation-application/", fd, {headers:{"Content-Type":"multipart/form-data"}});
      onSuccess();
    } catch(err:any) {
  ...
  return (
    <Box>
      <Box sx={{mb:2.5}}>
        <Typography sx={{fontWeight:800,fontSize:17,color:C.g900,lineHeight:1.3}}>Book this Package</Typography>
      ...
      <Divider sx={{mb:2}}>
        <Typography sx={{fontSize:11,fontWeight:700,color:C.g400,textTransform:"uppercase",letterSpacing:"0.6px"}}>Vacation details</Typography>
      </Divider>

      <Box sx={{display:"flex",flexDirection:"column",gap:1.5,mb:2.5}}>
        <TextField label="Destination" size="small" fullWidth value={form.destination} sx={SX_INPUT}
          placeholder={offer?.destination??offer?.location??"e.g. Dubai, UAE"}
          onChange={e=>setForm(p=>({...p,destination:e.target.value}))}/>
        <TextField label="Travel Date *" type="date" size="small" fullWidth
          value={form.travel_date} InputLabelProps={{shrink:true}} sx={SX_INPUT}
          onChange={e=>setForm(p=>({...p,travel_date:e.target.value}))}/>
        <TextField label="Number of Travellers *" type="number" size="small" fullWidth
          value={form.number_of_people} inputProps={{min:1,max:50}} sx={SX_INPUT}
          onChange={e=>setForm(p=>({...p,number_of_people:e.target.value}))}/>
      ...
      <Button variant="contained" fullWidth onClick={handleSubmit}
        disabled={submitting||!form.travel_date||!form.accommodation_type}
        ...
        {submitting?"Submitting…":allReady?"Book Now":"Book Now Anyway"}
      </Button>
      ...
      {!allReady && (
        <Box sx={{display:"flex",alignItems:"center",justifyContent:"center",gap:0.5,mt:1.5}}>
          ...
          <Typography sx={{fontSize:12,color:C.g400}}>to strengthen this booking</Typography>
        </Box>
      )}
    </Box>
  );
...
const VacationDetails: React.FC = () => {
  ...
  useEffect(()=>{
    if (!id) return;
    Promise.all([
      api.get(`/app/vacation-offer/${id}/`).then(r=>r.data),
    ...
    }).catch(()=>setError("Failed to load vacation details.")).finally(()=>setLoading(false));
  },[id]);

  useEffect(()=>{
    if (!id||!user) return;
    setAppLoading(true);
    api.get("/app/vacation-application/",{params:{offer_id:id}})
      .then(res=>{
        const results=Array.isArray(res.data?.results)?res.data.results:Array.isArray(res.data)?res.data:[];
        const found=results.find((a:any)=>String(a.vacation_offer??a.offer?.id??a.offer)===String(id));
  ...
  useEffect(()=>{
    if (!offer) return;
    const desc=offer.description||offer.vacation_description||"";
  ...
  const META_COLS=[
    {label:"Destination",    value:capitalizeWords(str(offer.destination??offer.location))},
    {label:"Duration",       value:str(offer.duration)},
    {label:"Price",          value:offer.price!=null?`£${Number(offer.price).toLocaleString()}`:"N/A"},
    {label:"Departure Date", value:fmtDate(offer.departure_date??offer.start_date)},
    {label:"Return Date",    value:fmtDate(offer.return_date??offer.end_date)},
    {label:"Max Group Size", value:str(offer.max_group_size??offer.available_slots)},
  ];
  ...
  return (
    <Box sx={{p:1,width:"100%"}}>
      <Breadcrumbs sx={{mb:2}}>
        ...
        <Typography color="text.primary">Vacation Details</Typography>
      </Breadcrumbs>
      ...
      <Box sx={{display:"flex",flexDirection:{xs:"column",md:"row"},gap:4,px:{xs:0,md:2}}}>
        ...
        <Box sx={{flex:2,minWidth:0}}>
          <Card sx={{borderRadius:"18px",boxShadow:"0 2px 12px rgba(0,0,0,.07)",mb:3}}>
            <CardContent sx={{p:3}}>
              {(offer.cover_image||offer.image) && (
                <CardMedia component="img" image={offer.cover_image??offer.image} alt="Vacation"
                  sx={{width:"100%",height:200,borderRadius:"12px",objectFit:"cover",mb:2.5}}/>
              )}
              <Typography variant="h5" sx={{fontWeight:800,color:C.g900,mb:0.5}}>
                {capitalizeWords(offer.offer_title??offer.title??offer.name??"Vacation Package")}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{mb:2}}>
                {offer.destination??offer.location??""}
              </Typography>
        ...
        <Box sx={{flex:1,alignSelf:"flex-start",bgcolor:"#fff",borderRadius:"18px",
          ...
          {appLoading ? (
            ...
            <Box sx={{textAlign:"center",py:3}}>
              ...
              <Typography sx={{fontWeight:800,fontSize:18,color:C.g900,mb:0.75}}>Booking Submitted!</Typography>
              <Typography sx={{fontSize:13,color:C.g500,mb:3}}>Your vacation booking has been received.</Typography>
